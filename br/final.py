...
class MyGazu:
    ...
    # def new_task2(self, layout):
    #     self.task_type = gazu.task.get_task_type_by_name(layout)
    #     self.task = gazu.task.new_task(self.asset, self.task_type)
    #     pp.pprint(self.task)
    #
    #
    ...
    def get_output_path_for_shot(self, sequence, shot, output_type, t_type):
        ...
        if output.get('name').lower() not in seq_ext:
            output_ext = f".{output.get('name').lower()}"
        else:
            output_ext = f".{self.get_frame_padding()}.{output.get('name').lower()}"
        ...
        # output_file_path = os.path.dirname(
        #     gazu.files.build_entity_output_file_path(pick_shot, output, task_type, revision=last_revision)
        # )
